export const data = {
  word: "change",
  phonetic: "/tʃeɪndʒ/",
  phonetics: [
    {
      text: "/tʃeɪndʒ/",
      audio: "https://api.dictionaryapi.dev/media/pronunciations/en/change-uk.mp3",
      sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=15170531",
      license: { name: "BY-SA 3.0", url: "https://creativecommons.org/licenses/by-sa/3.0" },
    },
    {
      text: "/tʃeɪndʒ/",
      audio: "https://api.dictionaryapi.dev/media/pronunciations/en/change-us.mp3",
      sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=840607",
      license: { name: "BY-SA 3.0", url: "https://creativecommons.org/licenses/by-sa/3.0" },
    },
  ],
  meanings: [
    {
      partOfSpeech: "noun",
      definitions: [
        {
          definition: "The process of becoming different.",
          synonyms: [],
          antonyms: [],
          example: "The product is undergoing a change in order to improve it.",
        },
        {
          definition: "Small denominations of money given in exchange for a larger denomination.",
          synonyms: [],
          antonyms: [],
          example: "Can I get change for this $100 bill please?",
        },
        { definition: "A replacement, e.g. a change of clothes", synonyms: [], antonyms: [] },
        {
          definition:
            "Balance of money returned from the sum paid after deducting the price of a purchase.",
          synonyms: [],
          antonyms: [],
          example:
            "A customer who pays with a 10-pound note for a £9 item receives one pound in change.",
        },
        {
          definition:
            "Usually coins (as opposed to paper money), but sometimes inclusive of paper money",
          synonyms: [],
          antonyms: [],
          example: "Do you have any change on you? I need to make a phone call.",
        },
        {
          definition: "A transfer between vehicles.",
          synonyms: [],
          antonyms: [],
          example: "The train journey from Bristol to Nottingham includes a change at Birmingham.",
        },
        { definition: "A change-up pitch.", synonyms: [], antonyms: [] },
        {
          definition:
            "(campanology) Any order in which a number of bells are struck, other than that of the diatonic scale.",
          synonyms: [],
          antonyms: [],
        },
        {
          definition: "A place where merchants and others meet to transact business; an exchange.",
          synonyms: [],
          antonyms: [],
        },
        { definition: "A public house; an alehouse.", synonyms: [], antonyms: [] },
      ],
      synonyms: ["transformation", "transition"],
      antonyms: [],
    },
    {
      partOfSpeech: "verb",
      definitions: [
        {
          definition: "To become something different.",
          synonyms: [],
          antonyms: [],
          example: "The tadpole changed into a frog.   Stock prices are constantly changing.",
        },
        {
          definition: "To make something into something else.",
          synonyms: [],
          antonyms: [],
          example:
            "The fairy changed the frog into a prince.   I had to change the wording of the ad so it would fit.",
        },
        {
          definition: "To replace.",
          synonyms: [],
          antonyms: [],
          example:
            "Ask the janitor to come and change the lightbulb.   After a brisk walk, I washed up and changed my shirt.",
        },
        {
          definition: "To replace one's clothing.",
          synonyms: [],
          antonyms: [],
          example:
            "You can't go into the dressing room while she's changing.   The clowns changed into their costumes before the circus started.",
        },
        {
          definition: "To replace the clothing of (the one wearing it).",
          synonyms: [],
          antonyms: [],
          example: "It's your turn to change the baby.",
        },
        {
          definition: "To transfer to another vehicle (train, bus, etc.)",
          synonyms: [],
          antonyms: [],
        },
        { definition: "To exchange.", synonyms: [], antonyms: [] },
        {
          definition: "To change hand while riding (a horse).",
          synonyms: [],
          antonyms: [],
          example: "to change a horse",
        },
      ],
      synonyms: ["alter", "another", "make", "modify", "transform"],
      antonyms: [],
    },
  ],
  license: { name: "CC BY-SA 3.0", url: "https://creativecommons.org/licenses/by-sa/3.0" },
  sourceUrls: ["https://en.wiktionary.org/wiki/change"],
};

export default data;
